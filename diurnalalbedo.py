...
class albedo:
    ...
    def load_parameters(self,soil_model:Sequence[float],location:tuple,errors:Sequence[float]=[]) -> None:
        ...
        colnames = []
        colnames += ['Day','Date','Mean_am_albedo','Mean_pm_albedo','Max_error_%']
    ...
    @property
    def pm_half_length_of_the_day(self):
        return self._utm_time['sunset']-self._utm_time['noon']
    
    @property
    def am_half_length_of_the_day(self):
    ...
    def _calculate_day(self):
        ...
        amhd = self.am_half_length_of_the_day.seconds
        pmhd = self.pm_half_length_of_the_day.seconds
                
        amseconds = -np.arange(amhd)
        pmseconds = np.arange(pmhd)
        amelevations = np.array([self._angle(int(second)) for second in amseconds])
        pmelevations = np.array([self._angle(int(second)) for second in pmseconds])
        above = np.where((amelevations>=0) &  (amelevations <=90))[0]
        amelevations = amelevations[above]
        above = np.where((pmelevations>=0) &  (pmelevations <=90))[0]
        pmelevations = pmelevations[above]

        self._albedos = {}
        self._albedos['am'] = self._aTS(amelevations)
        self._albedos['pm'] = self._aTS(pmelevations)
        self._mean_albedos = {}
        self._mean_albedos['am'] = self._albedos['am'].mean()
        self._mean_albedos['pm'] = self._albedos['pm'].mean()
        self._mean_albedos['day'] = np.concatenate((self._albedos['am'],self._albedos['pm'])).mean()
    ...
    def describe_day(self) -> tuple:
        """Returns tuple with:
            ...
            - full length of the day
            - time of sunrise (UTM)
            - morning length of the day
            - time of noon (UTM)
            - afternoon length of the day
            - time of sunset (UTM)

        Returns:
            tuple: tuple with day description
        """
        return self.print_current_date(),self._angle(0), \
            str(self.full_length_of_the_day), \
            self._utm_time['sunrise'].strftime("%H:%M:%S"), \
            str(self.morning_length_of_the_day), \
            self._utm_time['noon'].strftime("%H:%M:%S"), \
            str(self.afternoon_length_of_the_day), \
    ...
    def get_mean_albedo(self) -> float:
        ...
        """
        return self._mean_albedos['day']   
   
    def get_min_albedo(self) ->float:
        return min(self._albedos['am'].min(),self._albedos['pm'].min())
    ...
    def _am_albedo_time_delta(self,value:float) -> datetime.timedelta:
        if value < self.get_min_albedo():
            return None
        below_value = np.where(self._albedos['am']<value)[0].max()
        return -datetime.timedelta(0,int(below_value)) #negative
    ...
    def get_albedo_time_delta(self,timeofday:str) -> datetime.timedelta:
        ...
        Args:
            ...
            timeofday (str): am or pm ['am','pm']

        Returns:
            datetime.timedelta or None: timedelta from the noon or None if value smaller than minimum albedo
        """   
        if timeofday not in ['am','pm']:
            print("timeofday: am, pm")
            return None
        
        return self._pm_albedo_time_delta(self._mean_albedos['pm']) if timeofday == 'pm' else self._am_albedo_time_delta(self._mean_albedos['am'])
    ...
    def get_albedo_main_times(self) -> tuple:
        ...
        amvalue = self._mean_albedos['am']
        pmvalue = self._mean_albedos['pm']

        utm_am_time = self._utm_time['noon'] + self._am_albedo_time_delta(amvalue)
        utm_pm_time = self._utm_time['noon'] + self._pm_albedo_time_delta(pmvalue)
        slt_am_time = self._slt_time['noon'] + self._am_albedo_time_delta(amvalue)
        slt_pm_time = self._slt_time['noon'] + self._pm_albedo_time_delta(pmvalue)
        ...
        #am errors towards sunrise
        errors = self._errors
        for error in errors[::-1]:
            val = amvalue*(1+error)
            tim = self._twelve_noon+self._am_albedo_time_delta(val)
            values+=[val]
            times+=[tim.time()]
    ...
    def get_day_record(self,header:bool=False) -> pd.Series:
        ...
        am_mean = round(self._mean_albedos['am'],4)
        pm_mean = round(self._mean_albedos['pm'],4)
        max_error = round(self.get_albedo_max_error(),4)
        record = w[0]+[am_mean,pm_mean,max_error]+w[1]+w[2]
        if header:
    ...
    def plot_time_curve(self,figure:Union[plt.figure,str]=None,suptitle:str='') -> None:
        ...
        am_mean_albedo_value = self._mean_albedos['am']
        pm_mean_albedo_value = self._mean_albedos['pm']
        ...
        slt_am_time = self._slt_time['noon'] + self._am_albedo_time_delta(am_mean_albedo_value)
        slt_pm_time = self._slt_time['noon'] + self._pm_albedo_time_delta(pm_mean_albedo_value)
        ...
        ax0.axhline(y = am_mean_albedo_value,color=mean_color_H,linestyle="--",alpha=0.5,label="mean albedo value")
        ...
        ax1.axhline(y = pm_mean_albedo_value,color=mean_color_H,linestyle="--",alpha=0.5)
