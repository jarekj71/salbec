...
class soilDatabase():
    ...
    def removeFromDatabase(self,soilName): # not work yet
        with self._soilName(soilName) as soilName:
            database = self._getDatabase()
            database.pop(soilName)
            os.remove(os.path.join(self._soilDir,soilName+'.sl'))
            pickle.dump(database,open(self._databaseFile,"wb+"))
            return True
        return False
...
class soil():
    ...
    def importSeries(self,wavelengths,reflectance):
        ...
        self._f = interp1d(wavelengths,reflectance,kind="quadratic")
        self._gl = {}
